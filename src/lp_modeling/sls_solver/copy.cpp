    ...
    struct opt_solver::imp {
        ...
        long double hard_score(int m_var, long double m_shift) {
            ...
            for(auto cons: m_constraints) {
                ...
                } else if(!pre_sat && post_sat) { // case II. unsat --> sat
                    if(cons->m_lower && cons->m_upper) {
                ...
                } else if(!pre_sat && !post_sat) { // case III. unsat --> unsat, only consider two bounds' case
                    if(cons->m_lower && cons->m_upper) {
                        if(pre_sum < cons->m_lower_bound && post_sum > cons->m_upper_bound) { // lower unsat --> upper unsat
                            score += cons->get_lower_weight() * 1.0;
                            score -= cons->get_upper_weight() * 1.0;
                        } else if(pre_sum > cons->m_upper_bound && post_sum < cons->m_lower_bound) { // upper unsat --> lower unsat
                            score += cons->get_upper_weight() * 1.0;
                            score -= cons->get_lower_weight() * 1.0;
                        } else { // other cases, don't consider

                        }
                    }
                } else { // case IV. sat --> sat, don't consider
        ...
        long double soft_score(int m_var, long double m_shift) const {
            ...
            return obj_move == 0 ? 0 :
                    (obj_move < 0 ? 1.0 * m_object_weight : -1.0 * m_object_weight);
        }
        ...
        void execute_critical_move(int m_var, long double m_shift) {
            ...
            for(auto idx: m_vars[m_var].m_constraints) {
                ...
                if(pre_sat && !post_sat) { // sat --> unsat
                    m_unsat_constraints.insert(cons->get_index());
                } else if(!pre_sat && post_sat) { // unsat --> sat
                    m_unsat_constraints.erase(cons->get_index());
                }
                if(pre_bound && !post_bound) { // bound --> unbound
                    m_unbounded_constraints.insert(cons->get_index());
                } else if(!pre_bound && post_bound) { // unbound --> bound
        ...
        */
        void random_walk_sat() {
            ...
            if(m_operation_table.empty()) { // no operation
                no_operation_random_walk_unbound();
            } else { // choose the best operation
                int v_idx;
        ...
        void get_smallest_vnd_feasible_int_cons(constraint * cons, int v1, int v2, long double t_coeff1, long double t_coeff2, bool & exist, bool & inf, long double & val) {
            if(cons->m_lower && cons->m_upper) {
                bool exist1, inf1, exist2, inf2; long double val1, val2;
        ...
        void get_smallest_vnd_feasible_int_cons_lower(constraint * cons, int v1, int v2, long double t_coeff1, long double t_coeff2, bool & exist, bool & inf, long double & val) {
            ...
            if(cons->m_poly.m_vars.count(v1) != 0 && cons->m_poly.m_vars.count(v2) != 0) {
                ...
                if(cons->m_poly.m_var_vars[v1].first.count(v2) != 0) { // Factor with each other
                    exist = false;
                    return;
                }
                ...
                if(t_coeff == 0) {
                    ...
                    exist = true; inf = false; val = std::ceil(diff / t_coeff);
                } else {
                    exist = true; inf = true;
                }
                ...
                if(t_coeff == 0) {
                    ...
                    exist = true; inf = true; val = std::ceil(diff / t_coeff);
                } else {
                    exist = true; inf = true;
                }
            } else {
        ...
        void get_smallest_feasible_int_value_cons(int v, constraint const* cons, int& val, bool& inf, bool& exist) {
            if (cons->m_lower && cons->m_upper) {  // get larger one
        ...
        void insert_and_select_paired_lift_moves(int & v1, int & v2, long double & shift1, long double & shift2, long double & score) {
            ...
            for(int obj_v: m_vars_in_obj_without_bound) {
                ...
                for(auto idx: m_vars[obj_v].m_constraints) {
                    ...
                    for(int other_v: cons->m_poly.m_var_vars.at(obj_v).second) {
                        ...
                        if(cons->m_lower) {
                            if(m_vars[obj_v].m_coeff > 0) { // we make change to let obj_v get smaller
                                if(coeff1 > 0) { // obj_v >= (lb - coeff2 * other_v) / coeff1
                                    if(coeff_ratio > 0) { // we make other_v smaller
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(down);
                                    } else { // we make other_v larger
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(up);
                                    }
                                } else { // obj_v <= goal, do not consider

                                }
                            } else { // we make change to let obj_v get larger
                                if(coeff1 > 0) { // obj_v >= goal, do not consider

                                } else { // obj_v <= (lb - coeff2 * other_v) / coeff_1
                                    if(coeff_ratio > 0) { // we make other_v larger
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(up);                                        
                                    } else { // we make other_v smaller
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(down);
                                    }
                        ...
                        if(cons->m_upper) {
                            if(m_vars[obj_v].m_coeff > 0) { // we make change to let obj_v smaller
                                if(coeff1 > 0) { // obj <= goal, do not consider

                                } else { // obj >= (ub - coeff2 * other_v) / coeff1
                                    if(coeff_ratio > 0) { // we make other_v smaller
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(down);
                            ...
                            } else { // we make change to let obj_v larger
                                if(coeff1 > 0) { // obj <= (ub - coeff2 * other_v) / coeff1
                                    if(coeff_ratio > 0) { // we make other_v larger
                                        curr_vars.insert(other_v);
                                        m_operated_vars.push_back(std::make_pair(other_v, obj_v));
                                        m_operated_dirs.push_back(up);
            ...
            for(int i = 0; i < cnt; i++) {
                ...
                if(curr_score > score) {
                    v1 = other_v; v2 = obj_v;
                    shift1 = other_shift; shift2 = obj_shift;
                    score = curr_score;
                }
            }
        }
        ...
        */
        ...
        // // Gradient Descent
        // void calculate_gradient_vector() {
        //     m_gradient_vector.resize(m_vars.size());
        //     for(auto idx: m_unsat_constraints) {
        //         auto cons = m_constraints[idx];
        //         auto sum = cons->m_poly.get_sum();
        //         gradient_vector curr_vec = cons->m_poly.generate_gradient(m_vars.size());
        //         if(!cons->get_lower_sat()) { // sum is too small, we want it larger
        //             auto dscore = cons->m_lower_bound - sum;
        //             m_gradient_vector.add(curr_vec * dscore * learning_rate);
        //         }
        //         if(!cons->get_upper_sat()) { // sum is too big, we want it smaller
        //             auto dscore = sum - cons->m_upper_bound;
        //             m_gradient_vector.add(curr_vec * -dscore * learning_rate);
        //         }
        //     }
        //     learning_rate *= laerning_rate_decay;
        //     gradient_step++;
        // }

        // void execute_gradient_move() {
        //     for(int v = 0; v < m_vars.size(); v++) {
        //         execute_critical_move(v, m_gradient_vector.m_values[v]);
        //     }
        // }

        // vnd_operation_table m_vnd_operation_table;

        // VND
        // void insert_vnd_move_from_unsat_level_one() {
        //     m_vnd_operation_table.clear();
        //     const int pair_nums = 100; // each unsat clause can insert pair_nums at most
        //     int cons_vnd_num;
        ...
        //         cons_vnd_num = 0;
        //         auto cons = m_constraints[idx];
        //         for(int v1: cons->m_poly.m_vars) {
        //             // std::cout << "factor size: " << cons->m_poly.m_var_vars[v1].first.size() << std::endl;
        //             // std::cout << "intercept size: " << cons->m_poly.m_var_vars[v1].second.size() << std::endl;
        //             cons->m_poly.store_factor_and_intercept_vars(v1);
        //             for(int v2: cons->m_poly.m_var_vars[v1].second) { // loop intercept vars
        //                 if(cons->m_poly.m_var_vars[v1].first.count(v2) != 0) { // Ignore factor vars
        //                     continue;
        //                 }
        //                 if(v1 < v2) {
        //                     int shift1, shift2;
        //                     if(calculate_vnd_shift_from_constraint(cons, v1, v2, shift1, shift2)) {
        //                         enter_num++;
        //                         cons_vnd_num++;
        //                         m_vnd_operation_table.insert({v1, v2}, {shift1, shift2});
        //                     }
        //                     else {
        //                         nonenter_num++;
        //                     }
        //                 }
        //                 if(cons_vnd_num >= pair_nums) {
        //                     break_flag = true;
        //                     break;
        //                 }
        //             }
        ...
        //         }
        //     }
        //     std::cout << "enter and nonenter: " << enter_num << " " << nonenter_num << std::endl; 
        // }

        // void insert_vnd_move_from_unsat_level_two() {
        //     m_vnd_operation_table.clear();
        //     for (auto idx : m_unsat_constraints) {
        //         auto cons = m_constraints[idx];
        //         for (int v1 : cons->m_poly.m_vars) {
        //             for (int v2 : cons->m_poly.m_var_vars[v1].first) { // insert from factor vars
        //                 if(cons->m_poly.m_var_vars[v1].second.count(v2) != 0) { // Ignore intercept vars
        //                     continue;
        //                 }
        //                 auto curr_pair = std::make_pair(v1, v2);
        //                 if (v1 < v2) {
        //                     int shift1, shift2;
        //                     if (calculate_vnd_shift_from_constraint(cons, v1, v2, shift1, shift2)) {
        //                         m_vnd_operation_table.insert({v1, v2}, {shift1, shift2});
        ...
        // bool calculate_vnd_shift_from_constraint(constraint * cons, int v1, int v2, int & shift1, int & shift2) {
        //     auto sum = cons->m_poly.get_sum();
        //     auto coeff1 = cons->m_poly.m_var_factor_value[v1], coeff2 = cons->m_poly.m_var_factor_value[v2];
        //     if(coeff1 == 0 || coeff2 == 0) {
        //         return false;
        //     }
        //     // coeff1 * v1 + coeff2 * v2 == lower bound
        //     if(!cons->get_lower_sat()) {
        //         auto diff = cons->m_lower_bound - sum;
        //         diff /= 2;
        //         auto real_shift1 = diff / coeff1, real_shift2 = diff / coeff2;
        //         if(is_int(real_shift1)) {
        //             shift1 = real_shift1;
        //         } else {
        //             shift1 = coeff1 > 0 ? std::ceil(real_shift1) : std::floor(real_shift1);
        //         }
        //         if(is_int(real_shift2)) {
        //             shift2 = real_shift2;
        //         } else {
        //             shift2 = coeff2 > 0 ? std::ceil(real_shift2) : std::floor(real_shift2);
        //         }
        //         // std::cout << 
                
        //         bool res = check_var_shift(v1, shift1) && check_var_shift(v2, shift2);
        //         // if(res) std::cout << "enter" << std::endl;
        //         return res;
        //     }
        //     // coeff1 * v1 + coeff2 * v2 == upper bound
        //     if(!cons->get_upper_sat()) {
        //         auto diff = cons->m_upper_bound - sum;
        //         diff /= 2;
        //         auto real_shift1 = diff / coeff1, real_shift2 = diff / coeff2;
        //         if (is_int(real_shift1)) {
        //             shift1 = real_shift1;
        //         } else {
        //             shift1 = coeff1 > 0 ? std::floor(real_shift1) : std::ceil(real_shift1);
        //         }
        //         if (is_int(real_shift2)) {
        //             shift2 = real_shift2;
        //         } else {
        //             shift2 = coeff2 > 0 ? std::floor(real_shift2) : std::ceil(real_shift2);
        //         }
        //         bool res = check_var_shift(v1, shift1) && check_var_shift(v2, shift2);
        //         // std::cout << (res ? "enter\n" : "not enter\n");
        //         // if(res) std::cout << "enter" << std::endl;
        ...
        // void select_best_vnd_move(int & v1, int & v2, long double & shift1, long double & shift2, long double & score) {
        //     score = INT32_MIN;
        //     int size = m_vnd_operation_table.size(), cnt;
        //     std::cout << " m_vnd_operation_table.size: " << m_vnd_operation_table.size() << std::endl;
        //     bool BMS;
        //     if(size > VND_BMS_VAL) {
        //         BMS = true; cnt = VND_BMS_VAL;
        //     } else {
        //         BMS = false; cnt = size;
        //     }
        //     int curr_v1, curr_v2;
        ...
        //         if(BMS) {
        //             int rand_index = rand() % (size - i);
        //             curr_v1 = m_vnd_operation_table.m_var_vector[rand_index][0];
        //             curr_v2 = m_vnd_operation_table.m_var_vector[rand_index][1];
        //             curr_shift1 = m_vnd_operation_table.m_shift_vector[rand_index][0];
        //             curr_shift2 = m_vnd_operation_table.m_shift_vector[rand_index][1];
        //             m_vnd_operation_table.m_var_vector[rand_index] = m_vnd_operation_table.m_var_vector[size - i - 1];
        //             m_vnd_operation_table.m_shift_vector[rand_index] = m_vnd_operation_table.m_shift_vector[size - i - 1];
        //         } else {
        //             curr_v1 = m_vnd_operation_table.m_var_vector[i][0];
        //             curr_v2 = m_vnd_operation_table.m_var_vector[i][1];
        //             curr_shift1 = m_vnd_operation_table.m_shift_vector[i][0];
        //             curr_shift2 = m_vnd_operation_table.m_shift_vector[i][1];
        //         }
        //         curr_score = calculate_vnd_score(curr_v1, curr_v2, curr_shift1, curr_shift2);
        //         if(curr_score > score) {
        //             score = curr_score;
        //             v1 = curr_v1;
        //             v2 = curr_v2;
        ...
        //         }
        //     }
        //     std::cout << "best score vnd : " << score << std::endl;
        // }

        // long double calculate_vnd_score(int v1, int v2, long double shift1, long double shift2) {
        //     long double score = 0;
        //     int_table m_looped_cons;
        //     m_assignment.shift(v1, shift1);
        //     m_assignment.shift(v2, shift2);
        //     for(auto idx: m_vars[v1].m_constraints) {
        //         auto cons = m_constraints[idx];
        //         auto pre_sum = cons->m_poly.get_sum();
        //         auto post_sum = cons->m_poly.calculate_sum(m_assignment);
        //         auto pre_sat = std::make_pair(cons->get_lower_sat(), cons->get_upper_sat());
        //         auto post_sat = check_constraint_sat_using_sum(cons, post_sum);
        //         score += cons->get_lower_weight() * (post_sat.first - pre_sat.first);
        //         score += cons->get_upper_sat() * (post_sat.second - pre_sat.second);
        //         m_looped_cons.insert(idx);
        //     }
        //     for(auto idx: m_vars[v2].m_constraints) {
        //         if(m_looped_cons.count(idx) != 0) {
        //             continue;
        //         }
        //         auto cons = m_constraints[idx];
        ...
        //     m_assignment.shift(v1, -shift1);
        //     m_assignment.shift(v2, -shift2);
        //     score += soft_score(v1, shift1) + soft_score(v2, shift2);
        //     return score;
        // }
        ...
        void solve() {
            ...
            for(m_step = 0; m_step <= max_step; m_step++) {
                ...
                if(is_sat_status) {
                    if(!enable_all_lift_move_pool) {
                    ...
                    else { // Put life move, paired lift move and vnd lift move in a pool
                        ...
                        // if(single_score <= 0 && vnd_score <= 0 && pair_score <= 0) { // no positive score exists
                        //     int var_index;
                    ...
                    if(enable_vnd_search) { // enable vnd version
